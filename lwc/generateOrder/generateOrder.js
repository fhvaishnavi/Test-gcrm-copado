        ...
        getContracts({ oppId: this.recordId})
            .then(result => {
                ...
                if(this.contractRecords > 0 && orderedFlag == false){
                    ...
                    this.showToastMessage('ERROR!', 'Please verify the Opportunity has an active Contract before generating the order. ', 'error');
                }
            })
            .catch(error => {
                this.showToastMessage('Error', 'Please check if the Opportunity has an active Contract before generating the order', 'error');
                console.error('Error fething contract records:', 'error');
            });
        }
